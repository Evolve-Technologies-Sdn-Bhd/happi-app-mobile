/**
 * Home Asset Edit Screen
 * Mirrors happi-app-customer/src/views/profile/family-assets/home/edit.vue
 * Fields: Name, Home Category, Address (multiline), Postcode, State, Agree checkbox
 * Buttons: Remove Home (edit mode) + Save
 */
...
type RoutePropType = RouteProp<ProfileStackParamList, 'HomeAssetEdit'>;
...
export const HomeAssetEditScreen: React.FC = () => {
  ...
  const [picker, setPicker] = useState<PickerSheet>({
    visible: false, title: '', field: 'category', options: [], loading: false,
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getHomeInfo(homeId);
  ...
  const doSave = async () => {
    if (!form.address1.trim()) {
      Alert.alert('Validation', 'Address is required'); return;
    }
    setSaving(true);
    try {
      const payload = {
      ...
      if (isAdd) {
        ...
        await updateHome({ ...payload, id: form.id });
      }
      navigation.goBack();
    } catch {
  ...
  // ── Remove ───────────────────────────────────────────────────────────────────
  const doRemove = () => {
    Alert.alert('Reminder', 'Confirm to remove this home?', [
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <Header title={isAdd ? 'Add Home' : 'Home Details'} showBack />
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.form}>
          ...
          <TouchableOpacity
            style={styles.row}
            onPress={() => openPicker('category', 'HOME_CATEGORY', 'Select Home Category')}
          >
            <Text style={styles.label}>Home Category</Text>
            <View style={styles.rowRight}>
              <Text style={styles.value}>{getLabel('HOME_CATEGORY', form.category)}</Text>
          ...
          <TouchableOpacity
            style={styles.row}
            onPress={() => openPicker('stateName', 'STATE_NAME', 'Select State')}
          >
            <Text style={styles.label}>State</Text>
            <View style={styles.rowRight}>
              <Text style={styles.value}>{getLabel('STATE_NAME', form.stateName)}</Text>
              <Ionicons name="chevron-forward" size={16} color="#D3D4D6" />
            </View>
          </TouchableOpacity>
          <View style={styles.divider} />
          ...
          >
            ...
            <Text style={styles.agreeText}>
              ...
              &quot;My Housing&quot;, and I understand I can withdraw my content anytime.
            </Text>
          </TouchableOpacity>
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 8 }]}>
        {!isAdd && (
          <TouchableOpacity style={styles.removeBtn} onPress={doRemove}>
            <Text style={styles.removeBtnText}>Remove Home</Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity
